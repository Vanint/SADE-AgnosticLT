...
class LT_Dataset(Dataset):
    ...
    def __init__(self, root, txt, transform=None, train=False):
        self.img_path = []
        self.train= train
        self.labels = []
        self.transform = transform
        self.test_trsfm = transforms.Compose([
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            orginal_sample = self.test_trsfm(sample)
            sample = self.transform(sample)
        # return sample, label, path
        if self.train:
            return sample, label, orginal_sample
        else:
            return sample, label
...
class ImageNetLTDataLoader(DataLoader): 
    ...
    def __init__(self, data_dir, batch_size, shuffle=True, num_workers=1, training=True, balanced=False, retain_epoch_size=True, 
        ...
        if training:
            ...
            train_dataset = LT_Dataset(data_dir, test_txt, TwoCropsTransform(train_trsfm), train=True)
...
def main(config):
    ...
    for test_distribution in test_distribution_set:    
        ...
        weight_record = test_training_on_streaming_data(train_data_loader, model, aggregation_weight, optimizer, num_classes, config, args)
       
        print("Final aggregation weight: Expert 1 is {0:.2f}, Expert 2 is {1:.2f}, Expert 3 is {2:.2f}".format(weight_record[0], weight_record[1], weight_record[2]))    
...
def test_training_on_streaming_data(train_data_loader,model,  aggregation_weight, optimizer,   num_classes, config, args):
    model.eval() 
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') 
    ...
    for i, (data, target, original_data) in enumerate(tqdm(train_data_loader)):
        ...
        with torch.no_grad():
            ...
            original_data, target = original_data.to(device), target.to(device)
    ...
    print("This epoch: Many-shot {0:.2f}, Medium-shot {1:.2f}, Few-shot {2:.2f}, All {3:.2f}".format(many_shot_acc * 100, medium_shot_acc * 100,  few_shot_acc * 100, eval_acc_mic_top1* 100))     
